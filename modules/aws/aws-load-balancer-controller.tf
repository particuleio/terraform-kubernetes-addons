...
serviceAccount:
  ...
  annotations:
    eks.amazonaws.com/role-arn: "${local.aws-load-balancer-controller["enabled"] && local.aws-load-balancer-controller["create_iam_resources_irsa"] ? module.iam_eks_role_aws-load-balancer-controller.iam_role_arn : ""}"
VALUES
}

module "iam_eks_role_aws-load-balancer-controller" {
  ...
  cluster_service_accounts = {
    "${var.cluster-name}" = [
      "${local.aws-load-balancer-controller["namespace"]}:${local.aws-load-balancer-controller["service_account_name"]}"
    ],
  }
  tags = local.tags
}
