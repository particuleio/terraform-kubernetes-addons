...
=======
| <a name="input_ip-mask-agent"></a> [ip-mask-agent](#input\_ip-mask-agent) | Configure ip mask agent chart, see `ip-mask-agent.tf` for supported values. This addon works only on GCP. | `any` | `{}` | no |
>>>>>>> ce1440a (feat: add IP mask agent addons for google)
